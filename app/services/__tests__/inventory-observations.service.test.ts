...
type InventoryObservationFormData = Parameters<typeof addInventoryObservation>[0];
...
describe("inventory-observations.service", () => {
  ...
  describe("getInventoryObservationsByItemId", () => {
    it("should return empty array when itemId is empty", async () => {
      const result = await getInventoryObservationsByItemId("");
      expect(result).toEqual([]);
      expect(mockGet).not.toHaveBeenCalled();
    });

    it("should fetch observations by inventory item id", async () => {
      mockGet.mockResolvedValue(mockObservations);

      const result = await getInventoryObservationsByItemId("item-1");

      expect(mockGet).toHaveBeenCalledWith("/inventory-items/item-1/observations");
  ...
  describe("getInventoryObservationById", () => {
    it("should return undefined when id is undefined", async () => {
      const result = await getInventoryObservationById(undefined);
    ...
    it("should fetch observation by id", async () => {
      ...
      const result = await getInventoryObservationById("obs-1");

      expect(mockGet).toHaveBeenCalledWith("/inventory-observations/obs-1");
    ...
    it("should return undefined on 404 error", async () => {
      ...
      const result = await getInventoryObservationById("nonexistent");
  ...
  describe("addInventoryObservation", () => {
    it("should create new observation", async () => {
      ...
      const result = await addInventoryObservation(formData);

      expect(mockPost).toHaveBeenCalledWith("/inventory-items/item-1/observations", {
        observation: "New observation",
        fileIds: ["file-4"],
      });
    ...
    it("should handle error", async () => {
      ...
      await expect(addInventoryObservation(formData)).rejects.toThrow();
    });
  });
  ...
  describe("updateInventoryObservation", () => {
    it("should update observation", async () => {
      ...
      const result = await updateInventoryObservation("obs-1", updateData);

      expect(mockPut).toHaveBeenCalledWith("/inventory-observations/obs-1", {
        observation: "Updated observation",
      });
    ...
    it("should handle error", async () => {
      ...
      await expect(updateInventoryObservation("nonexistent", updateData)).rejects.toThrow();
    });
  });
  ...
  describe("deleteInventoryObservation", () => {
    it("should delete observation", async () => {
      ...
      await deleteInventoryObservation("obs-1");

      expect(mockDelete).toHaveBeenCalledWith("/inventory-observations/obs-1");
    });

    it("should handle error", async () => {
      mockDelete.mockRejectedValue(new ApiError("Not Found", 404));

      await expect(deleteInventoryObservation("nonexistent")).rejects.toThrow();
    });
  });
});
