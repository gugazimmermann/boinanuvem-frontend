...
describe("EditEmployee", () => {
  ...
  const createRouter = (employeeId: string) => {
    return createMemoryRouter(
      ...
      {
        initialEntries: [`/dashboard/employees/${employeeId}/edit`],
      }
    );
  };
  ...
  it("should render edit employee form with pre-filled data", async () => {
  ...
  it("should handle form submission", async () => {
    vi.mocked(updateEmployee).mockReturnValue(true);
    const router = createRouter("employee-1");
    render(<RouterProvider router={router} />);
  ...
  it("should have correct meta function", () => {
    expect(EditEmployee).toBeDefined();
  });
  ...
  it("should mask CEP, CPF, and phone inputs", async () => {
    const router = createRouter("employee-1");
    render(<RouterProvider router={router} />);
    ...
    expect(zipCodeInput || cpfInput || phoneInput).toBeTruthy();
  });
