import { describe, it, expect, vi, beforeEach } from "vitest";
import { render, screen, fireEvent } from "@testing-library/react";
import { createMemoryRouter, RouterProvider } from "react-router";
import { LanguageProvider } from "~/contexts/language-context";
import { ThemeProvider } from "~/contexts/theme-context";
import NewSale from "../records.sales.new";
import { SaleType, PricingMode, SalePaymentMethod } from "~/types";

const mockNavigate = vi.fn();
const mockLocation = { state: null };

vi.mock("react-router", async () => {
  const actual = await vi.importActual("react-router");
  return {
    ...actual,
    useNavigate: () => mockNavigate,
    useLocation: () => mockLocation,
  };
});

vi.mock("~/services/sales.service", () => ({
  addSale: vi.fn(() => ({ id: "new-sale" })),
  isAnimalSold: vi.fn((id: string) => id === "animal-sold"),
}));

vi.mock("~/mocks/animals", async () => {
  const actual = await vi.importActual<typeof import("~/mocks/animals")>("~/mocks/animals");
  return actual;
});

vi.mock("~/services/animals.service", () => ({
  getAnimalsByCompanyId: vi.fn(() => [
    {
      id: "animal-1",
      code: "A001",
      registrationNumber: "REG001",
      status: "active",
      companyId: "company-1",
    },
    {
      id: "animal-2",
      code: "A002",
      registrationNumber: "REG002",
      status: "active",
      companyId: "company-1",
    },
    {
      id: "animal-sold",
      code: "A003",
      registrationNumber: "REG003",
      status: "sold",
      companyId: "company-1",
    },
  ]),
  getAnimalById: vi.fn((id: string) => {
    if (id === "animal-1") return { id: "animal-1", code: "A001", registrationNumber: "REG001" };
    if (id === "animal-2") return { id: "animal-2", code: "A002", registrationNumber: "REG002" };
    return undefined;
  }),
}));

vi.mock("~/services/weighings.service", () => ({
  getWeighingsByAnimalId: vi.fn(() => []),
}));

vi.mock("~/mocks/companies", () => ({
  mockCompanies: [{ id: "company-1", name: "Test Company" }],
}));

vi.mock("~/mocks/properties", () => ({
  mockProperties: [{ id: "property-1", name: "Test Property", companyId: "company-1" }],
}));

vi.mock("~/mocks/buyers", () => ({
  mockBuyers: [
    { id: "buyer-1", name: "Test Buyer", companyId: "company-1" },
    { id: "buyer-2", name: "Test Buyer 2", companyId: "company-1" },
  ],
}));

vi.mock("~/services/buyers.service", () => ({
  getBuyersByCompanyId: vi.fn(() => [
    { id: "buyer-1", name: "Test Buyer", companyId: "company-1" },
    { id: "buyer-2", name: "Test Buyer 2", companyId: "company-1" },
  ]),
}));

vi.mock("~/services/properties.service", () => ({
  getPropertiesByCompanyId: vi.fn(() => [
    { id: "property-1", name: "Test Property", companyId: "company-1" },
  ]),
}));

vi.mock("~/components/ui", () => ({
  Input: ({
    label,
    placeholder,
    value,
    onChange,
    type,
    ...props
  }: {
    label?: string;
    placeholder?: string;
    value?: string | number;
    onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void;
    type?: string;
    [key: string]: unknown;
  }) => (
    <input
      data-testid={`input-${label || placeholder || "input"}`}
      aria-label={label}
      placeholder={placeholder}
      value={value || ""}
      onChange={onChange}
      type={type}
      {...props}
    />
  ),
  Select: ({
    options,
    value,
    onChange,
    name,
    label,
    ...props
  }: {
    options?: Array<{ value: string; label: string }>;
    value?: string | number;
    onChange?: (e: React.ChangeEvent<HTMLSelectElement>) => void;
    name?: string;
    label?: string;
    [key: string]: unknown;
  }) => (
    <select
      data-testid={`select-${name || label || "select"}`}
      value={value || ""}
      onChange={onChange}
      {...props}
    >
      {options?.map((opt) => (
        <option key={opt.value} value={opt.value}>
          {opt.label}
        </option>
      ))}
    </select>
  ),
  Button: ({
    children,
    onClick,
    type,
    disabled,
    ...props
  }: {
    children?: React.ReactNode;
    onClick?: () => void;
    type?: "button" | "submit" | "reset" | undefined;
    disabled?: boolean;
    [key: string]: unknown;
  }) => (
    <button
      data-testid={type === "submit" ? "submit-button" : "button"}
      type={type as "button" | "submit" | "reset" | undefined}
      onClick={onClick}
      disabled={disabled}
      {...props}
    >
      {children}
    </button>
  ),
  Alert: ({ title, variant }: { title?: string; variant?: string }) => (
    <div data-testid={`alert-${variant}`}>{title}</div>
  ),
}));

describe("NewSale", () => {
  const createRouter = () => {
    return createMemoryRouter(
      [
        {
          path: "/dashboard/registros/vendas/novo",
          element: (
            <LanguageProvider>
              <ThemeProvider>
                <NewSale />
              </ThemeProvider>
            </LanguageProvider>
          ),
        },
      ],
      {
        initialEntries: ["/dashboard/registros/vendas/novo"],
      }
    );
  };

  beforeEach(() => {
    vi.clearAllMocks();
    mockLocation.state = null;
  });

  it("should render new sale form", () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const heading = screen.queryByRole("heading", { level: 1 });
    const buttons = screen.queryAllByRole("button");
    expect(heading || buttons.length > 0).toBeTruthy();
  });

  it("should handle form input changes", () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const dateInput =
      screen.queryByTestId("input-Data da Venda") || screen.queryByLabelText(/date/i);
    if (dateInput) {
      fireEvent.change(dateInput, { target: { value: "2024-01-15" } });
      expect(dateInput).toHaveValue("2024-01-15");
    }
  });

  it("should handle animal selection", () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const searchInput = screen.queryByPlaceholderText(/buscar animais/i);
    if (searchInput) {
      fireEvent.change(searchInput, { target: { value: "A001" } });
      expect(searchInput).toHaveValue("A001");
    }
  });

  it("should prevent selecting sold animals", () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const checkboxes = screen.queryAllByRole("checkbox");

    expect(checkboxes.length).toBeGreaterThanOrEqual(0);
  });

  it("should show validation errors on invalid submission", async () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const submitButtons = screen.queryAllByTestId("submit-button");
    const submitButton = submitButtons.find(
      (btn) =>
        (btn as HTMLButtonElement).type === "submit" ||
        btn.textContent?.includes("Registrar") ||
        btn.textContent?.includes("Register")
    ) as HTMLButtonElement | undefined;
    if (submitButton) {
      fireEvent.click(submitButton);
      const errors = screen.queryAllByText(/required|obrigatório/i);
      expect(errors.length >= 0).toBeTruthy();
    }
  });

  it("should handle pricing mode switching", () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const pricingModeSelect = screen.queryByTestId("select-Modo de Precificação");
    if (pricingModeSelect) {
      fireEvent.change(pricingModeSelect, { target: { value: PricingMode.TOTAL } });
      expect(pricingModeSelect).toHaveValue(PricingMode.TOTAL);
    }
  });

  it("should handle sale type selection", () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const saleTypeSelect = screen.queryByTestId("select-Tipo de Venda");
    if (saleTypeSelect) {
      fireEvent.change(saleTypeSelect, { target: { value: SaleType.SLAUGHTERHOUSE } });
      expect(saleTypeSelect).toHaveValue(SaleType.SLAUGHTERHOUSE);
    }
  });

  it("should handle payment method selection", () => {
    const router = createRouter();
    render(<RouterProvider router={router} />);

    const paymentSelect = screen.queryByTestId("select-Método de Pagamento");
    if (paymentSelect) {
      fireEvent.change(paymentSelect, { target: { value: SalePaymentMethod.CASH_FLOW } });
      expect(paymentSelect).toHaveValue(SalePaymentMethod.CASH_FLOW);
    }
  });

  it("should have correct meta function", () => {
    expect(NewSale).toBeDefined();
  });
});
