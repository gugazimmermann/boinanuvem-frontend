...
export default function EditSale() {
  ...
  // Check for already sold animals (excluding animals already in this sale)
  const checkForSoldAnimals = (animalIds: string[], currentSaleAnimalIds: string[]): string[] => {
    return animalIds.filter((id) => {
      // Allow animals that are already in this sale
  ...
  const toggleAnimalSelection = (animalId: string) => {
    // Get current sale's animal IDs to allow them
    const currentSaleAnimalIds = sale?.saleItems.map((item) => item.animalId) || [];

    // Prevent selecting already sold animals (unless they're already in this sale)
  ...
  const handlePricingModeChange = (value: PricingMode) => {
    setFormData((prev) => {
      ...
      if (value === PricingModeEnum.TOTAL && prev.totalPrice && prev.selectedAnimalIds.length > 0) {
        ...
        // Keep existing prices when switching to individual mode
      }
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.selectedAnimalIds.length === 0) {
      ...
      // Check if any selected animals are already sold (excluding current sale animals)
      const currentSaleAnimalIds = sale?.saleItems.map((item) => item.animalId) || [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        showAlert(t.sales?.success?.updated || "Venda atualizada com sucesso", "success");
        setTimeout(() => {
          navigate(getSaleViewRoute(sale.id));
        }, 1500);
      } else {
        showAlert(t.sales?.errors?.updateFailed || "Erro ao atualizar venda", "error");
      }
    } catch {
      showAlert(t.sales?.errors?.updateFailed || "Erro ao atualizar venda", "error");
    } finally {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/50 p-6 border border-gray-200 dark:border-gray-700">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {t.sales?.edit?.title || "Editar Venda"}
          </h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            {t.sales?.edit?.description || "Edite os dados da venda"}
          </p>
        </div>
          ...
          <div className="flex justify-end space-x-4">
            ...
            <Button type="submit" variant="primary" disabled={isSubmitting}>
              {isSubmitting
                ...
                : t.sales?.form?.update || "Atualizar Venda"}
            </Button>
          </div>
        </form>
      </div>
